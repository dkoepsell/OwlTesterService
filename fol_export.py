...
def render_prover9(theory, align_bfo=False):
    ...
    instance_of(x, C, t), even occurrents, so the export shares one predicate set
    with the BFO-2020 background theory (clif_theory.render_prover9_theory) and
    ...
    def memb_kind(iri):
        # When aligning to the BFO background, an occurrent instantiates via the
        # same ternary instance_of(x, C, t) as a continuant, so goals, ontology
        # axioms, and background all share one predicate.
        return "continuant" if align_bfo else kind(iri)
...
def render_clif(theory, align_bfo=False):
    """Render the theory in ISO Common Logic Interchange Format (CLIF).

    With align_bfo=True every class instantiates via the ternary
    (instance_of x C t), even occurrents, so the export shares one predicate set
    with the BFO-2020 background. The default align_bfo=False is byte-for-byte the
    standalone export (occurrents keep the binary (instance_of_at x O)).
    """
    head_note = (";; Aligned to the BFO background: all instantiation is ternary "
                 "(instance_of x C t)." if align_bfo else
                 ";; Continuants: (instance_of x C t)   Occurrents: (instance_of_at x O)")
