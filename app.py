...
def validate_ontology_completeness(analysis_id):
    ...
    try:
        ...
        ontology_file = OntologyFile.query.get(analysis.ontology_file_id)
        if not ontology_file:
            return jsonify({'success': False, 'message': 'Ontology file not found'}), 404
            
        # Construct full path to the file
        file_path = ontology_file.file_path
        ...
        try:
            ...
            if isinstance(result, tuple) and not result[0]:
                return jsonify({'success': False, 'message': f"Failed to load ontology: {result[1]}"}), 400
        
        # Run completeness validation
        completeness_result = custom_tester.validate_completeness()
        
        # Update analysis record with completeness data
        analysis.completeness = completeness_result
        db.session.commit()
        
        return jsonify({
            'success': True,
            'message': 'Completeness validation finished successfully',
...
def check_enhanced_consistency(analysis_id):
    ...
    try:
        ...
        consistency_result = custom_tester.check_enhanced_consistency()
        
        # Update analysis record with consistency data
        # Convert is_consistent field to boolean
        analysis.is_consistent = consistency_result.get('consistent', True)
        analysis.consistency_issues = consistency_result
        db.session.commit()
        
        return jsonify({
            'success': True,
            'message': 'Enhanced consistency check completed successfully',
...
def generate_implications(analysis_id):
    ...
    try:
        ...
        if request.method == 'GET':
            # Return existing implications if available
            if analysis.real_world_implications:
                return jsonify({
                    'success': True,
                    'implications': analysis.real_world_implications,
                    'generated_at': analysis.implications_generation_date.isoformat() if analysis.implications_generation_date else None
                })
            else:
                return jsonify({'success': False, 'message': 'No implications generated yet'}), 404
                
        elif request.method == 'POST':
            # Generate new implications
            data = request.json or {}
            num_implications = data.get('count', 5)  # Default to 5 implications
            comprehensive = data.get('comprehensive', False)  # Default to standard generation
            ...
            # Get domain classes for use in implications generation
            domain_classes = []
            if hasattr(analysis, 'domain_classes') and analysis.domain_classes:
                domain_classes = analysis.domain_classes
            else:
                # Extract top-level domain classes (excluding BFO classes)
                ont_analysis = custom_tester.analyze_ontology()
                domain_classes = [cls for cls in ont_analysis.get('classes', []) 
                                 if not (cls.startswith('BFO_') or cls.lower().startswith('bfo'))][:10]  # Limit to top 10
            
            # Generate implications
            implications = custom_tester.generate_real_world_implications(
                ontology_name=analysis.ontology_name or "Unknown Ontology",
                domain_classes=domain_classes,
            ...
            # Update analysis record with implications data
            analysis.real_world_implications = implications
            analysis.implications_generated = True
            analysis.implications_generation_date = datetime.datetime.utcnow()
            db.session.commit()
            ...
            return jsonify({
                'success': True,
                'message': 'Implications generated successfully',
                'implications': implications
            })
